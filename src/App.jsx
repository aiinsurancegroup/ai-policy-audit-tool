...
export default function App() {
  ...
  const sendToClient = async () => {
    ...
    setError('');
    // The token is minted on the server. It used to be built here from three
    // Math.random() draws -- a bearer credential generated by a predictable PRNG
    // in the environment least able to keep a secret.
    const resp = await adminApi(adminPw, { action: 'create_client_link', payload: {
      client_name: clientName, client_industry: clientInd, client_contact: clientContact,
      client_email: clientEmail, file_count: 0,
      named_insured: namedInsured.trim() || null, mailing_address: mailingAddress.trim() || null,
    }});
    if (!resp.ok) { setError('Failed to create audit.'); return; }
    const created = await resp.json().catch(() => null);
    if (!created?.audit_id || !created?.token) { setError('Failed to create audit.'); return; }
    const audit = { id: created.audit_id };
    await logActivity(adminPw, audit.id, 'CLIENT_LINK_CREATED', { client: clientName, expires_at: created.expires_at }, 'operator');
    const link = window.location.origin + '?token=' + created.token;
    setClientLink(link);
    await loadAudits();
    setClientName(''); setClientInd(''); setClientContact(''); setClientEmail(''); setNamedInsured(''); setMailingAddress('');
  };
